...
### 2026-09-03 — TDD: release-profile contract launcher

Test-only `bootstrap_and_contract_test_launchers_use_release_locked_offline_cargo` first failed meaningfully: the copied real launcher recorded `run --locked --offline --quiet -p plasmosome-work-state -- contract-test ...`, while the required transcript's first semantic mismatch was missing `--release`. The only green implementation change adds `--release` to the existing `contract-test` Cargo invocation, preserving exact `--locked --offline --quiet -p plasmosome-work-state --` ordering and forwarded arguments. The focused transcript test passed in 0.54 seconds and all 15 CLI tests passed in 3.52 seconds, including the unchanged ordinary installed-wrapper/no-Cargo and sync installed-wrapper/no-Cargo proofs. Full crate tests passed in 17.66 seconds; clippy with warnings denied, fmt check, and diff check passed. No Cargo manifest, lockfile, profile setting, Rust product/contract code, installed route, public command, or ordinary-read behavior changed.

The release-built physical contract path remains subject to the same pinned-artifact, no-write contract. Cold compile and warmed retained physical timings are intentionally deferred to the parent-owned final release invocations; coverage remains in its instrumented test profile and is unaffected by this launcher-only change.

### 2026-09-03 — TDD: pending metadata fixture follows the sealed private runtime

The first cold release-profile retained `online-sync` run at clean `884bc5a` refused before its scenario inventory with `cutover_blocked` (exit 1; `/usr/bin/time -p` `real 312.74`, `user 131.66`, `sys 52.55`). Temporary diagnostics, removed before this change, isolated the later pending-same retry to the contract transport rejecting a valid metadata-stage `bd --version` plan and collapsing that runner refusal to `unsupported_beads_version` (exit 2; `real 313.00`, `user 132.41`, `sys 55.97`). These runs are diagnostics, not acceptance evidence.

Test-only `fixture_runtime_environment_binds_private_beads_state_and_disables_backup` first failed because the exact fixture map lacked `BEADS_DIR` and `BD_BACKUP_ENABLED`. The green helper now binds only `<stage>/repository/.beads` and `false` in addition to the existing sealed runtime fields. Test-only `online_sync_contract_transport_admits_only_metadata_version_after_pending_refusal` then failed meaningfully at the valid metadata-stage predicate: the former matcher required `cwd=None`. Its green form admits only a distinct absolute direct sibling `.staging-<nonempty>/bd` with `argv=["--version"]`, `cwd` equal to that metadata stage, no redactions, and the complete helper-produced environment. It rejects the initial observation stage even with its own matching runtime, none/repository/outside cwd, malformed stage names, missing/altered/extra environment fields, altered `BEADS_DIR` or backup flag, wrong argv, and redactions before dispatch. Focused tests passed, followed by `cargo test -p plasmosome-work-state` (54 library, 1 binary, 15 CLI, 4 command, 40 contract, 17 document, 10 freshness, 10 pin, 1 project, 6 read, 15 shadow, 25 store, and 11 sync tests) in 20.60 seconds wall time; clippy with warnings denied, fmt check, and diff check passed. This is contract-fixture alignment only: no production Store/Sync/Pin behavior, command allowance, error mapping, remote write, credential, actor, or heartbeat behavior changed. A new retained release physical run remains required from the committed head.

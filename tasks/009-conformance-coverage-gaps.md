...
all. Two live grants sharing a handle make `plasmosome-ledger`'s replay silently leak one: the
first revoke consumes the handle, the second finds nothing to undo, and the capability stays
granted.
...
path the ledger takes for every compensating effect, so a backend can pass the suite and undo
nothing.

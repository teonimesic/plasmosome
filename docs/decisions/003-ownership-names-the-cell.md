...
wanting the cell back has only the key to read it out of — which some classes happen to encode
(§4 puts each cell's socket under `cells/<cell>/`) and others do not. That is the mangled name
again, arrived at from the other side. Where the two cells also share a class and key — a proxy
map for the same host, a mount on the same target — the objects deduplicate outright, and the
drift between them is not merely unattributed but gone.
...
and giving it up to keep an owner field narrow is the wrong trade. The residual contract cost is
small but real: `102` carries only `target`, so naming which cell already holds the plasmid wants
a new field on an existing code, and §1 forbids clients parsing `message` to recover it.
...
- **`OsObject`, `UniverseOp`, and what embeds them.** The owner field, and every constructor that
  fills it. `OsState`, `Diff` and `ResidueReport` all hold `OsObject` and change shape with it;
  every one of them is `wire_serde`-frozen in `plasmosome-freeze-checks`. `ResidueReport` is not
  internal — spec 001 §3.4 puts it on the wire as `plasmosome.stop`'s `residue_items`. So this
  option changes a frozen response body as well. The choice is not freeze-touching against
...
Two claims made while arguing for this did not survive checking, and are recorded so nobody
repeats them. Spec 001 §3.3's example does **not** show one plasmid in two cells — its `cell-2`
carries an empty plasmid list. And refusing a duplicate attach would **not** need a new error
code: `102` is already `already_attached`, and only the field naming the other cell is missing.
The case for this decision rests on the verb signatures and the per-attachment mock mode, not on
either of those.

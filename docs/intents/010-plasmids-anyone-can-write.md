...
cell is supervised — only what the new capability is and what it should let a cell do.
...
the plasmid code that fills the gap. That is the case I want to be easy, because it is the one
...
it — how a plasmid is declared, what the minimal contract for attaching one is, and what the gate
is — is still to be designed.

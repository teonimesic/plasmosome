...
pub fn isolated_environment(root: &std::path::Path) -> BTreeMap<String, String> {
    ...
    for name in [
        ...
        environment.insert(
            name.to_owned(),
            root.join(name.to_ascii_lowercase()).display().to_string(),
        );
    }
...
pub fn run_contract(request: &ContractRequest) -> Result<ContractResult, ContractResult> {
    if matches!(request.case.as_str(), "github" | "all") && request.github_remote.is_none() {
        return Err(ContractResult::refusal(
            &request.case,
            "github_fixture_required",
        ));
    }
    let root = tempfile::tempdir()
        .map_err(|_| ContractResult::refusal(&request.case, "cutover_blocked"))?;
    let manifest = PinManifest::load("tools/work-state-beads-1.1.2.toml")
        .map_err(|error| ContractResult::refusal(&request.case, error.code()))?;
    ...
    if request.case == "github" || request.case == "all" || remote_case(&request.case) {
        let remote = request
            .github_remote
            .as_deref()
            .ok_or_else(|| ContractResult::refusal(&request.case, "github_fixture_required"))?;
        if request.confirmation.as_deref() != Some("refs/dolt/data") {
            return Err(ContractResult::refusal(
                &request.case,
                "github_fixture_invalid",
            ));
        }
        let mut runner = SystemCommandRunner;
        fixture_preflight(remote, "refs/dolt/data", &mut runner)
            .map_err(|code| ContractResult::refusal(&request.case, code))?;
        return Err(ContractResult::refusal(&request.case, "cutover_blocked"));
    }
    if request.case == "stealth-init" || request.case == "hermetic" {
        run_stealth_init(&request.binary, root.path())
            .map_err(|code| ContractResult::refusal(&request.case, code))?;
    }
    Ok(ContractResult {
        ...
        code: "ok".to_owned(),
        beads_version: manifest.version,
        clone_labels: Vec::new(),
        observed_base: None,
        final_generation: None,
    })
...
fn remote_case(case: &str) -> bool {
    matches!(
        case,
        "stale-base-fence" | "push-conflict-recovery" | "transport-retries"
    )
}
fn valid_fixture(remote: &str) -> bool {
    let no_credentials = !remote.contains('@') && !remote.contains("//:");
    let github = remote.starts_with("https://github.com/")
...
pub fn fixture_preflight<R: CommandRunner>(
    remote: &str,
    reference: &str,
    runner: &mut R,
) -> Result<(), &'static str> {
    if !valid_fixture(remote) || reference != "refs/dolt/data" {
        return Err("github_fixture_invalid");
    }
    let output = runner
        .run(CommandSpec {
            program: PathBuf::from("git"),
            argv: vec!["ls-remote".into(), remote.to_owned(), reference.to_owned()],
            cwd: None,
            environment: BTreeMap::new(),
            redacted_argv_positions: Vec::new(),
        })
        .map_err(|_| "github_fixture_invalid")?;
    if output.status != 0 {
        return Err("github_fixture_invalid");
    }
    if !output.stdout.trim().is_empty() {
        return Err("github_fixture_not_empty");
    }
    Ok(())
}
...
pub fn clone_paths(root: impl AsRef<std::path::Path>) -> (PathBuf, PathBuf) {
    (root.as_ref().join("clone-a"), root.as_ref().join("clone-b"))
}
...
pub fn cleanup_command(remote: &str, generation: &str) -> CommandSpec {
    CommandSpec {
        ...
        argv: vec![
            "push".into(),
            remote.to_owned(),
            format!("--force-with-lease=refs/dolt/data:{generation}"),
            ":refs/dolt/data".into(),
        ],
        cwd: None,
        environment: BTreeMap::new(),
...
fn run_stealth_init(binary: &std::path::Path, root: &std::path::Path) -> Result<(), &'static str> {
    ...
    let output = runner
        .run(stealth_init_command(binary.to_path_buf(), repository, root))
        .map_err(|_| "cutover_blocked")?;
    if output.status != 0 {
        return Err("cutover_blocked");
    }
    let output = runner
        .run(CommandSpec {
            ...
            cwd: Some(root.join("stealth-repository")),
            environment,
            redacted_argv_positions: Vec::new(),
        })
        .map_err(|_| "cutover_blocked")?;

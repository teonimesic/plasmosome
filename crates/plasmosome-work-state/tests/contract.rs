...
use plasmosome_work_state::contract::{classify_push, fixture_preflight, PushFailure};

#[test]
fn a_fixture_with_credentials_wrong_name_or_existing_ref_is_refused_before_write() {
    for remote in ["https://token@github.com/acme/plasmosome-work-state-fixture.git", "https://github.com/acme/not-a-fixture.git"] {
    ...
    let mut runner = RecordingCommandRunner::with_output(CommandOutput::success("deadbeef\trefs/dolt/data\n"));
    let error = fixture_preflight("https://github.com/acme/plasmosome-work-state-fixture.git", "refs/dolt/data", &mut runner).unwrap_err();
...
fn cleanup_deletes_only_the_generation_this_run_owns() {
    let command = plasmosome_work_state::contract::cleanup_command("https://github.com/acme/plasmosome-work-state-fixture.git", "abc");
    assert_eq!(command.argv, vec!["push", "https://github.com/acme/plasmosome-work-state-fixture.git", "--force-with-lease=refs/dolt/data:abc", ":refs/dolt/data"]);
}

#[test]
fn stealth_init_uses_the_exact_non_integrating_command() {
    let command = plasmosome_work_state::contract::stealth_init_command("/tmp/bd", "/tmp/repo", "/tmp/root");
    assert_eq!(command.argv, vec!["--sandbox", "init", "--stealth", "--skip-agents", "--skip-hooks", "--non-interactive"]);
